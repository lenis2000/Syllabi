...
\begin{itemize}
	...
	\item \textbf{Quizzes}. 
		...
		The lowest two quiz grades out of 11 total quizzes are dropped.
...
\begin{enumerate}[\bf{}{[}week 1{]}]
	...
	\item 10/30[Q8], 11/1.
		...
		Joint distributions (\texttt{6.1-6.3}).
		Sums of independent random variables; convolution (\texttt{7.1}, \texttt{8.1}).
	\item 11/6[Q9], 11/8.
		\emph{PS8} due on Tuesday at 10pm. 
		Expectation and independence (\texttt{8.2}), covariance and correlation (\texttt{8.4}), bivariate normal distribution (\texttt{6.4} and \texttt{8.5}). Markov and Chebyshev inequalities (\texttt{9.1}).
