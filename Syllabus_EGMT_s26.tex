...
\date{Compiled on \today, \currenttime. An up to date syllabus is always at \href{https://github.com/lenis2000/Syllabi/blob/master/Syllabus_EGMT_s26.pdf}{\texttt{this link}}.}
\maketitle
...
\smallskip
\noindent\textbf{Course Info:} EGMT 1520-123 (19659) --- Empirical Engagement (Seminar).

\smallskip
\noindent\textbf{Weekly rhythm:} Class meets \textbf{Mondays and Wednesdays 5:00PM -- 6:15PM} in \textbf{Clark Hall 101}.
\begin{itemize}
...
Pencils and paper are provided in class.
...
This is a hands-on, pen-and-paper course. Since we meet twice a week, our workflow is designed to move from intuition to rigorous proof over the course of a ``Problem Cycle.''
...
\begin{enumerate}[$\bullet$]
    ...
    \begin{itemize}
        \item \textbf{The 15-Minute Primer:} We begin with a short lecture introducing the day's core concept (e.g., definitions, the Pigeonhole Principle, logic notation) so you have the tools you need before you start solving.
    ...
    \begin{itemize}
        \item \textbf{Model Proof Analysis:} We will review a ``Model Solution'' on the board to explicitly highlight expectations for structure, clarity, and logical flow.
        \item \textbf{Mini-Debates:} We will hold impromptu debates where groups present their partial solutions and the class acts as ``Skeptics'' to test the arguments.
        \item \textbf{Acceptance:} By the end of class (or the start of the next), your group aims to have your solution ``accepted'' by the instructor. A problem is ``accepted'' when your subgroup can answer follow-ups and defend key steps without unresolved gaps.
    \end{itemize}
\end{enumerate}

\subsection{Home subgroups (fixed)}
On day one we form 6 ``home subgroups'' for in-class collaboration:
...
The Commonplace Book is required for your weekly writing assignments and reflections, which you submit as photos to Canvas. It is helpful to bring it to class each time, but pencils and paper are provided.
...
\begin{itemize}
    \item \textbf{In-Class Acceptance:} Your goal is to get your group's solution ``accepted'' by the instructor by the following Wednesday class. An accepted solution has no unresolved logic gaps.
...
\medskip\noindent
Note: It is helpful to bring your Commonplace Book to class each time,
to keep your homework in one place, as well as to put important
advising information there. But pencils and paper are provided in class.
...
Consistent attendance and thoughtful in-class participation are absolutely essential for your successful completion of Engagements courses. Since each course is only 7 weeks long (with 14 class meetings), each absence is significant. That said, there are situations in which you may need to miss a class meeting. In those situations,
Engagements professors are asked to observe the following policy:

\begin{enumerate}[$\bullet$]
    \item (excused absences) Authorized university activities (such as university-sponsored athletic events) and the observance of religious holidays are considered excused absences. However, you must submit a written request detailing the dates of these absences directly to me by \textbf{January 20} so that you can be placed in a different section if needed. If these types of absences would result in you missing more than 2 of the required class meetings, you may need to switch to a different course with fewer scheduling conflicts.
